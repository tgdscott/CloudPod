...
def get_user_stats(session: Session, user_id: UUID) -> Dict[str, Any]:
    ...
    try:
        total_episodes_stmt = select(func.count(Episode.id)).where(Episode.user_id == user_id)
        total_episodes = session.exec(total_episodes_stmt).one_or_none() or 0
    except Exception:
        total_episodes = 0

    try:
        episodes_last_30d_stmt = select(Episode).where(Episode.user_id == user_id, Episode.status == EpisodeStatus.published)
        rows = session.exec(episodes_last_30d_stmt).all()
        episodes_last_30d = sum(1 for e in rows if getattr(e, 'publish_at', None) and e.publish_at >= thirty_days_ago)
    except Exception:
        episodes_last_30d = 0

    try:
        upcoming_stmt = select(Episode).where(Episode.user_id == user_id, Episode.status != EpisodeStatus.published)
        rows = session.exec(upcoming_stmt).all()
        upcoming_scheduled = sum(1 for e in rows if getattr(e, 'publish_at', None) and e.publish_at > now)
    except Exception:
        upcoming_scheduled = 0

    last_published_at = None
    try:
        ep_stmt = select(Episode).where(Episode.user_id == user_id, Episode.status == EpisodeStatus.published).order_by(Episode.publish_at.desc()).limit(1)
    ...
    try:
        recent_episode_stmt = select(Episode).where(Episode.user_id == user_id).order_by(Episode.processed_at.desc()).limit(1)
