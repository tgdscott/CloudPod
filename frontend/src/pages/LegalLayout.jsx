...
export default function LegalLayout({ title, description, html }) {
  const metaDescription = description || `Read the Podcast Plus ${title}.`;

  return (
    <div className="min-h-screen bg-slate-100 text-slate-900">
      <MetaHead title={`${title} | Podcast Plus`} description={metaDescription} />

      <header className="border-b bg-white">
        <div className="mx-auto flex max-w-5xl items-center justify-between px-6 py-4">
          <a href="/" className="flex items-center gap-3 text-slate-900 no-underline hover:opacity-80" aria-label="Podcast Plus home">
            <Logo size={32} />
          </a>
      ...
      <footer className="border-t bg-white">
        <div className="mx-auto flex max-w-5xl flex-col gap-3 px-6 py-6 text-sm text-slate-500 md:flex-row md:items-center md:justify-between">
          <span>© {currentYear} Podcast Plus. All rights reserved.</span>
